...
interface LiquidationFilters {
  protocols?: string[];
  minBonus?: number;
  minDebt?: string;
}

const LIQUIDATIONS_CHANNEL = '/liquidations/live';
...
export default function LiquidationsPage() {
    ...
    try {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (message.type === 'LIQUIDATION_ALERT') {
            setLiquidations(prev => [message.data, ...prev.slice(0, 49)]); // Keep last 50
          } else if (message.type === 'SUBSCRIBE_CONFIRMED') {
            setIsSubscribed(true);
            console.log('Subscribed to liquidations channel');
          } else if (message.type === 'UNSUBSCRIBE_CONFIRMED') {
            setIsSubscribed(false);
            console.log('Unsubscribed from liquidations channel');
  ...
  const subscribeToChannel = () => {
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      const subscribeMessage = {
        type: 'SUBSCRIBE',
        channel: LIQUIDATIONS_CHANNEL,
        filters: {
          ...(filters.protocols && filters.protocols.length > 0 && { protocols: filters.protocols }),
          ...(filters.minBonus && { minBonus: filters.minBonus }),
          ...(filters.minDebt && { minDebt: filters.minDebt })
        }
      };
  ...
  const formatUSD = (wei: string, decimals: number = 18) => {
    // For demo purposes, assuming the value is already in a reasonable unit
    // In real implementation, you'd need proper price conversion
  ...
  const columns = [
    ...
    },
    {
      ...
      header: 'User',
      render: (v: string) => (
        <span className="font-mono text-gray-300 text-xs" title={v}>
          {short(v)}
        </span>
      )
    },
    ...
    },
    {
      ...
      render: (v: number) => (
        <span className={`font-bold text-xs ${
          v < 1 ? 'text-red-400' : 'text-green-400'
        }`}>
          {v.toFixed(2)}
        </span>
      )
            ...
            <div className="h-full">
              ...
              <div className="bg-gray-900 border-b border-gray-800 px-1.5 py-0.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1">
                    ...
                    <h2 className="font-mono text-[8px] uppercase tracking-widest text-gray-300">LIQUIDATION ALERTS</h2>
                    <div className="px-1 py-0.5 bg-orange-900 text-orange-300 text-[7px] font-mono border border-orange-700">
                      {liquidations.length} ALERTS
                    </div>
                  </div>
              ...
              <div className="px-1.5 py-2 bg-black border-b border-gray-800">
                ...
                {showFilters && (
                  <div className="mt-2 p-2 bg-gray-900 border border-gray-700 rounded">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-[7px]">
                      <div>
                        <label className="text-gray-400 block mb-1">Protocols (comma-separated)</label>
                        <input
                          type="text"
                          placeholder="aave, compound"
                      ...
                      <div>
                        <label className="text-gray-400 block mb-1">Min Bonus %</label>
                        <input
                          type="number"
                          step="0.1"
                          value={filters.minBonus || ''}
                          onChange={(e) => setFilters(prev => ({ ...prev, minBonus: e.target.value ? parseFloat(e.target.value) : undefined }))}
                          className="w-full px-2 py-1 bg-gray-800 border border-gray-600 text-white font-mono text-[7px] rounded"
                        />
                      </div>
                      <div>
                        <label className="text-gray-400 block mb-1">Min Debt (USD)</label>
                        <input
                          type="text"
                          placeholder="1000"
                          value={filters.minDebt || ''}
                          onChange={(e) => setFilters(prev => ({ ...prev, minDebt: e.target.value || undefined }))}
                          className="w-full px-2 py-1 bg-gray-800 border border-gray-600 text-white font-mono text-[7px] rounded"
                        />
                      </div>
                    </div>
              ...
              <div className="p-1.5">
                <Table
                  data={liquidations}
                  columns={columns}
                  emptyMessage={isSubscribed ? "[WAITING FOR LIQUIDATION ALERTS...]" : "[SUBSCRIBE TO START MONITORING]"}
                  density="compact"
                />
              </div>
            </div>
